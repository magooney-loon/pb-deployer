...
export class DeploymentCrudClient extends BaseClient {
	...
	async createDeployment(data: { app_id: string; version_id: string; status?: string }): Promise<Deployment> {
	...
	async updateDeployment(id: string, data: Partial<{ status: string; logs: string; started_at: string; completed_at: string }>): Promise<Deployment> {
