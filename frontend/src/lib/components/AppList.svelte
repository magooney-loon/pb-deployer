...
<div class="p-6">
	...
	{#if getAvailableServers().length === 0 && !showCreateForm}
		<div class="mb-6 rounded-lg border border-yellow-200 bg-yellow-50 p-4">
			<div class="flex">
				...
				<div class="ml-3">
					<h3 class="text-sm font-medium text-yellow-800">No Ready Servers</h3>
	...
	{#if showCreateForm}
		<div class="mb-6 rounded-lg bg-white p-6 shadow">
			<h2 class="mb-4 text-xl font-semibold">Add New Application</h2>
			<form
				onsubmit={(e) => {
					e.preventDefault();
					createApp();
				...
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div>
						...
						<input
							...
							placeholder="my-app"
						/>
						<p class="mt-1 text-xs text-gray-500">Used for directory and service naming</p>
					</div>
					<div>
						<label for="server-select" class="block text-sm font-medium text-gray-700">Server</label
						>
						<select
							id="server-select"
					...
					<div class="md:col-span-2">
						...
						/>
						<p class="mt-1 text-xs text-gray-500">The domain where your app will be accessible</p>
					</div>
					<div>
						<label for="remote-path" class="block text-sm font-medium text-gray-700"
	...
	{:else if apps.length === 0}
		<div class="py-12 text-center">
			<p class="mb-4 text-lg text-gray-500">No applications created yet</p>
			...
			{:else}
				<p class="text-sm text-gray-400">Set up a server first before creating apps</p>
			{/if}
		</div>
	{:else}
								...
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="flex items-center">
										<div>
											...
											<div class="text-xs text-gray-400">{app.service_name}</div>
										</div>
									</div>
								</td>
								<td class="px-6 py-4 whitespace-nowrap">
									<div class="text-sm text-gray-900">{getServerName(app.server_id)}</div>
									<div class="text-xs text-gray-500">{app.remote_path}</div>
								</td>
								<td class="px-6 py-4 whitespace-nowrap">
									<span
