...
{#if state.open}
	...
	>
		...
		<div class="flex min-h-full items-center justify-center p-4">
			<div
				class="relative w-full {logic.getSizeClass()} max-h-[90vh] overflow-hidden rounded-lg border border-gray-200 bg-white shadow-2xl dark:border-gray-800 dark:bg-gray-900"
				role="dialog"
				aria-modal="true"
				tabindex="-1"
			...
			>
				...
				{#if state.title || state.closeable}
					...
					>
						...
						{#if state.closeable}
							<button
								onclick={() => logic.close()}
								class="rounded-md p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300"
				...
				{#if footer}
					<div
						class="border-t border-gray-200 bg-gray-50 px-6 py-4 dark:border-gray-800 dark:bg-gray-800/50"
					>
						{@render footer?.()}
					</div>
				{/if}
