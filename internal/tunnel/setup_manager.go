...
func (s *SetupManager) InstallCaddy() error {
	...
	if err == nil && result.ExitCode == 0 {
		...
		cmds := []string{
			...
			"bash -c \"set -o pipefail; curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg\"",
			"bash -c \"set -o pipefail; curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list\"",
			"apt update",
			"apt install -y caddy",
		}
